//PR 133/2020 Luka Ivkovic, Termin vezbi: Srijeda 14:15. Radjeno: SOV-A. Komentari: ...
    ...
    public:
        ...
        void infoMEM(){
            while(true){
                ...
                u32 freeMem = 0;
                vector<u32> phragment_lenght;
                for(list<Fragment>::const_iterator it = freeMemory.begin(); it!= freeMemory.end();++it){
                    freeMem += (*it).len;

                }
                char valueMem;
                u32 helpVar = freeMem;
                if((helpVar/(1024*1024))>= 1){
                    valueMem = 'M';
                    helpVar = helpVar/(1024*1024);
                }else if((helpVar/1024)>= 1){
                    valueMem = 'K';
                    helpVar = helpVar/1024;
                }else{
                    valueMem = 'B';
                }

                vector<u32> vecLength;
                int n = freeMemory.size();
                vecLength.resize(n);
                vector<u32>::iterator itvl= vecLength.begin();
                for(list<Fragment>::const_iterator it = freeMemory.begin(); it!= freeMemory.end();++it, ++itvl){
                    *itvl = it->len;
                }
                u32 best_region = *max_element(vecLength.begin(), vecLength.end());
                u32 mem4K = freeMem/4096;
                u32 help4K = alfa_region/4096;

                chad.reportFreeSpace(mem4K,helpVar, valueMem, help4K);
            }
        }
...
void func_partA(SystemMemory& mem){
    mem.infoMEM();
}
